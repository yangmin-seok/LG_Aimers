...
def _parse_torch_device(value) -> torch.device | None:
...
def get_eval_device(model) -> torch.device:
    # IMPORTANT: input tensors must follow the model's actual input embedding device.
    # For quantized / device-mapped models, forcing CUDA can create device mismatch errors.
    input_embed = getattr(model, "get_input_embeddings", lambda: None)()
    if input_embed is not None and hasattr(input_embed, "weight"):
        weight_device = getattr(input_embed.weight, "device", None)
    ...
    if isinstance(hf_device_map, dict) and hf_device_map:
        for key in ("model.embed_tokens", "embed_tokens", "transformer.wte"):
            if key in hf_device_map:
                parsed = _parse_torch_device(hf_device_map[key])
                if parsed is not None:
                    return parsed
        for mapped in hf_device_map.values():
            parsed = _parse_torch_device(mapped)
            if parsed is not None:
                return parsed
...
def get_preferred_device() -> torch.device:
    if PREFER_CUDA:
        ...
        if REQUIRE_CUDA:
            raise RuntimeError("CUDA is required (PREFER_CUDA=True, REQUIRE_CUDA=True) but not available.")
    return torch.device("cpu")
...
def load_model(dtype: torch.dtype, device_map=None, model_id: str | None = None):
    ...
    if PREFER_CUDA:
        embed_dev = get_eval_device(model)
        if embed_dev.type != "cuda":
            raise RuntimeError(
                "Model was expected on CUDA but input embedding is on "
                f"{embed_dev}. Check device_map/offload settings."
            )
    return model
